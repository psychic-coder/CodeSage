...
def process_repository(
    self, project_id: str, source_type: str, source_path: str, job_id: str = None
):
    ...
    try:
        loop.run_until_complete(
            _async_process(self, project_id, source_type, source_path, job_id)
        )
    finally:
        loop.close()
...
async def _async_process(
    task, project_id: str, source_type: str, source_path: str, job_id: str = None
):
    redis = get_redis()
    ...
    try:
        ...
        file_count = await run_ingestion_pipeline(
            project_id, source_type, source_path, report_progress
        )
...
async def _update_project_status(
    project_id: str, status: str, file_count: int = 0, error: str = None
):
