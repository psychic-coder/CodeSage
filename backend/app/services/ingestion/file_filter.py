...
SKIP_DIRECTORIES = {
    ...
    'env', 'dist', 'build', '.next', 'coverage', '.nyc_output', 'target',
    'vendor', '.idea', '.vscode'
}
SKIP_PATTERNS = [
    '*.min.js', '*.min.css', '*.map', '*.lock', 'package-lock.json', 'yarn.lock',
    'poetry.lock', 'Pipfile.lock', '*.pyc', '*.pyo', '*.class', '*.jar',
    '*.exe', '*.dll', '*.so', '*.jpg', '*.jpeg', '*.png', '*.gif', '*.svg',
    '*.mp4', '*.mp3', '*.pdf', '*.zip', '*.tar', '*.log', '*.cache'
]
ALWAYS_INCLUDE = {
    'package.json', 'requirements.txt', 'pyproject.toml', 'Cargo.toml',
    'go.mod', 'pom.xml', 'Makefile', 'Dockerfile', 'docker-compose.yml', 'README.md'
}
    ...
    for dirpath, dirnames, filenames in os.walk(root):
        ...
        for fname in filenames:
            ...
            if fname in ALWAYS_INCLUDE:
                accepted.append(fpath); continue
