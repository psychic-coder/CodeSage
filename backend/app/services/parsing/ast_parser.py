...
from app.services.parsing.tree_sitter_wrapper import (
    parse_generic,
    parse_javascript,
    parse_python,
)
...
def _parse_js(src: str, r: dict):
    ...
    for m in re.finditer(r'export\s+.*\s+from\s+["\']([^"\']+)["\']', src):
        r["imports"].append(m.group(1))
    for m in re.finditer(
        r"export\s+(?:default\s+)?(?:function|class|const|let|var)\s+(\w+)", src
    ):
    ...
    r["complexity"] = (
        len(re.findall(r"\b(if|else|for|while|switch|catch|\?|&&|\|\|)\b", src)) + 1
    )
...
def _parse_py(src: str, r: dict):
    ...
    r["complexity"] = (
        len(re.findall(r"\b(if|elif|else|for|while|except|and|or)\b", src)) + 1
    )
...
def _parse_html(src: str, r: dict):
    ...
    for m in re.finditer(
        r'<(?:script|link|iframe|source)\b[^>]*(?:src|href)=\s*["\']([^"\']+)["\']',
        src,
        re.IGNORECASE,
    ):
        imp = m.group(1).strip()
    ...
    r["complexity"] = (
        len(re.findall(r"<(?!/)(?!meta\b)(?!link\b)(?!img\b)[a-zA-Z][^>]*>", src)) + 1
    )
