        ...
        for node in walk(root):
            ...
            if t == "import_statement":
                ...
                for c in node.children:
                    if c.type == "string":
                        val = (
                            src_bytes[c.start_byte : c.end_byte]
            ...
            if t == "call_expression":
                ...
                if callee is not None:
                    callee_text = src_bytes[callee.start_byte : callee.end_byte].decode(
                        "utf8", errors="ignore"
                    )
                    ...
                    if callee_text.strip().startswith("require"):
                        ...
                        for ch in node.children:
                            if ch.type == "string":
                                val = (
                                    src_bytes[ch.start_byte : ch.end_byte]
                                    .decode("utf8", errors="ignore")
                                    .strip("\"'")
                                )
                                imports.append(val)
            ...
            if t in ("function_declaration", "method_definition"):
                ...
                for c in node.children:
                    if c.type == "identifier" or c.type == "property_identifier":
                        ...
                        break
                name = (
                    src_bytes[name_node.start_byte : name_node.end_byte].decode(
                        "utf8", errors="ignore"
                    )
                    if name_node
                    else "<anon>"
    ...
    try:
        ...
        for node in walk(root):
            ...
            if t == "call":
                ...
                callee_text = (
                    src_bytes[callee_node.start_byte : callee_node.end_byte].decode(
                        "utf8", errors="ignore"
                    )
                    if callee_node
                    else "<unknown>"
            ...
            if "function" in t or "method" in t or "func" in t:
                ...
                for c in node.children:
                    if "identifier" in c.type or "name" in c.type:
                        name = src_bytes[c.start_byte : c.end_byte].decode(
                            "utf8", errors="ignore"
                        )
                        break
                functions.append(
