...
async def list_projects(cu=Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    rows = (await db.execute(select(Project).where(Project.user_id == cu["user_id"]))).scalars().all()
    return rows


@router.post("", response_model=ProjectResponse)
async def create_project(req: ProjectCreate, cu=Depends(get_current_user), db: AsyncSession = Depends(get_db)):
...
async def get_project(project_id: str, cu=Depends(get_current_user), db: AsyncSession = Depends(get_db)):
...
async def update_project(project_id: str, req: ProjectUpdate, cu=Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    p = (await db.execute(select(Project).where(Project.id == project_id, Project.user_id == cu["user_id"]))).scalar_one_or_none()
    if not p: raise HTTPException(404, "Project not found")
...
async def delete_project(project_id: str, cu=Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    p = (await db.execute(select(Project).where(Project.id == project_id, Project.user_id == cu["user_id"]))).scalar_one_or_none()
    if not p: raise HTTPException(404, "Project not found")
